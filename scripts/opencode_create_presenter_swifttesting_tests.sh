...
if [[ -z "$PRESENTER_NAME" ]]; then
  echo "Usage: bash scripts/opencode_create_presenter_swifttesting_tests.sh <root_dir> <PresenterName> [output_file]"
  exit 1
fi
...
if [[ "$PRESENTER_NAME" == "LoginScreenPresenter" ]]; then
  if [[ -n "$OUT_FILE" ]]; then
    bash "$ROOT_DIR/scripts/opencode_create_login_presenter_swifttesting_tests.sh" "$ROOT_DIR" >/dev/null
    mkdir -p "$(dirname "$OUT_FILE")"
    cp "$ROOT_DIR/TestingTaskTests/LoginScreenPresenterSwiftTestingTests.swift" "$OUT_FILE"
    echo "DONE: $OUT_FILE"
  else
    bash "$ROOT_DIR/scripts/opencode_create_login_presenter_swifttesting_tests.sh" "$ROOT_DIR"
  fi
  exit 0
fi

if [[ -z "$OUT_FILE" ]]; then
  OUT_FILE="$ROOT_DIR/TestingTaskTests/${PRESENTER_NAME}SwiftTestingTests.swift"
fi
...
SWIFT_HEADER
  ...
  for method in "${METHODS[@]}"; do
    ...
    cat <<SWIFT_METHOD
    @Test("TODO: cover ${method}")
    func test_${sanitized_method}_TODO() {
        #expect(Bool(false), "TODO: implement ${PRESENTER_NAME}.${method} behavior checks")
    }
...
private extension ${PRESENTER_NAME}SwiftTestingTests {
